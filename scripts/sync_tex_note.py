...
HOME_START_MARKER = "<!-- HOME-NOTES:START -->"
HOME_END_MARKER = "<!-- HOME-NOTES:END -->"
...
CATEGORY_CONFIG = {
    "dft": {
        ...
        "home_more_label": "Browse all DFT notes",
    },
    "tb": {
        ...
        "home_more_label": "Browse all tight-binding notes",
    },
    "other": {
        ...
        "home_more_label": "Browse all other notes",
    },
}
...
def latest_note_entries(
    entries: list[NoteEntry],
    limit: int = NOTES_PREVIEW_LIMIT,
) -> list[NoteEntry]:
    return sorted(
        entries,
        key=lambda entry: dt.datetime.fromisoformat(entry.created),
        reverse=True,
    )[:limit]
...
def replace_managed_block(index_html: str, new_block: str) -> str:
    ...
    return marker_pattern.sub(lambda _: new_block, index_html, count=1)
...
def build_overview_block(language: str = "en") -> str:
    ...
    for category, config in CATEGORY_CONFIG.items():
        ...
        for entry in latest_note_entries(entries):
...
def build_home_notes_block() -> str:
    lines = [HOME_START_MARKER, '        <div class="home-notes-directory">']
    for category, config in CATEGORY_CONFIG.items():
        category_href = html.escape(
            config["index_path"].relative_to(ROOT).as_posix(),
            quote=True,
        )
        lines.extend(
            [
                f'          <article class="home-notes-category" data-home-notes-category="{category}">',
                f'            <h3><a href="{category_href}">{html.escape(config["overview_label"])}</a></h3>',
                '            <div class="home-notes-entries">',
            ]
        )
        for entry in latest_note_entries(read_category_entries(category)):
            original_path = config["index_path"].parent / f"{entry.slug}.html"
            original_lang, alternate_filename, _ = note_page_metadata(original_path)
            note_path = (
                original_path
                if original_lang == "en"
                else original_path.parent / alternate_filename
            )
            _, _, title = note_page_metadata(note_path)
            note_href = html.escape(
                note_path.relative_to(ROOT).as_posix(),
                quote=True,
            )
            lines.append(
                f'              <a class="home-notes-entry" href="{note_href}">{html.escape(title)}</a>'
            )
        lines.extend(
            [
                "            </div>",
                f'            <a class="home-notes-more" href="{category_href}" aria-label="{html.escape(config["home_more_label"], quote=True)}">…</a>',
                "          </article>",
                "",
            ]
        )
    if lines[-1] == "":
        lines.pop()
    lines.extend(["        </div>", f"        {HOME_END_MARKER}"])
    return "\n".join(lines)
...
def update_notes_overview() -> list[Path]:
    ...
    for path, language in ((NOTES_OVERVIEW_PATH, "en"), (NOTES_OVERVIEW_ZH_PATH, "zh-CN")):
        ...
        overview_block = build_overview_block(language)
        path.write_text(
            marker_pattern.sub(lambda _: overview_block, source, count=1),
            encoding="utf-8",
        )
...
def update_home_notes() -> Path:
    if not HOME_PATH.exists():
        raise RuntimeError(f"首页不存在：{HOME_PATH}")
    source = HOME_PATH.read_text(encoding="utf-8")
    marker_pattern = re.compile(
        re.escape(HOME_START_MARKER)
        + r".*?"
        + re.escape(HOME_END_MARKER),
        re.DOTALL,
    )
    if not marker_pattern.search(source):
        raise RuntimeError(f"首页未找到受管 Notes 区块标记：{HOME_PATH}")
    home_block = build_home_notes_block()
    HOME_PATH.write_text(
        marker_pattern.sub(lambda _: home_block, source, count=1),
        encoding="utf-8",
    )
    return HOME_PATH
...
def update_note_catalog() -> Path:
    ...
    NOTE_SCRIPT_PATH.write_text(
        pattern.sub(lambda _: block, source, count=1),
        encoding="utf-8",
    )
...
def update_index(category: str, slug: str, title: str, created_iso: str) -> Path:
    ...
    sorted_entries = sorted(
        entries.values(),
        key=lambda item: (
            dt.datetime.fromisoformat(item.created),
            item.title,
        ),
        reverse=True,
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"[OK] Updated homepage notes: {home_path}")
